...
## 6. Build 계획 (비교란 additive)
1. **doctor 먼저**(read-only) — 즉시 유용(지금 환경도 바로 진단), 결정론 테스트.
2. **setup 확장**(repo_root_env + allowlist) — setup.py + setup.md.
각 단계 overlay-gates 무회귀(셋업/진단은 파이프라인 동작 무관).
