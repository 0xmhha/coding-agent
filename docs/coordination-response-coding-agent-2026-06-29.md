...
4. **차원(1024 vs 상향)은 coding-agent 무관** — 팩 형상·필드만 불변이면 OK. dim은 CKV/CKS/sqlite-vec
   합의 사항(CKS는 1024 유지 선호로 회신 — coding-agent도 *연속성* 관점에서 1024 유지에 약하게 동의).
...
## ★ 결정 필요 — 구현 전 3자 합의 (안 닫으면 PR-77 측정 무효/충돌)

협의 검토 중 식별된 갭. CKV 문서 §3-R에도 동일 항목으로 제기함(`coordination-prompts-2026-06-29.md`).

| # | 결정 | 왜 (안 닫으면) | 누가 |
|---|---|---|---|
| **D-1** ★ | Phase 2 재인덱싱 커밋을 **`0bf2f4d1b`(PR-77 버그-부모)로 통일**. CKG 매칭률 그래프 커밋 미지정 → 통일 제안(1회 재인덱싱 3자 커버) | 다른 커밋 인덱스면 **PR-77 A/B 측정 무효** | CKG/CKV 합의 |
| **D-2** | 재인덱싱 그래프 cache SchemaVersion **≥1.19(현 1.22)** 보장; coding-agent도 그 그래프에 cks 배선 | canonical_id 값 NULL → **join 조용히 실패** | 채택 확인 |
| **D-3** | parity 분리: recall/rerank툴=cks proxy 불요 / **flow·invariant=cks 표면 노출 필요(미구현)** | "15툴 배선" 오추적·우선순위 오판 | ✅ 우리측 정리 |
| **D-4** | `get_invariant_enforcement`(코드-도출 구현 불변식·H 가드레일 enabler) **Phase 2 스케줄 vs post-Phase-2 defer** 결정 | 미결 시 H 가드레일 **무기한 블록** | cks/ckv 결정 |
| **D-5** | ckg #40 R06 precision change가 graph-gap **P3**(suffix-resolver) supersede 여부 | 중복작업/오추적 | CKG 확인 |
