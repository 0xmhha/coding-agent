...
1. **임베딩 A/B = PR-77 Phase 2 런**: ckg/ckv 정리·재빌드·재인덱싱(핀 `0bf2f4d1b`) 완료 통지 받으면,
   coding-agent 재설치(v0.1.41) + bge-m3/Qwen3 before-after를 동일 코퍼스·태스크로 측정.
2. **flow/invariant 3자 인터페이스 공동설계**: 위 표 시그니처를 출발점으로 CKV·CKS와 확정. **CKS 표면
   노출**을 deliverable에 명시(없으면 coding-agent 미사용).
3. **schema_version**: serviceable-우선 + major-compare 보조 파싱을 analyzer §3.0b에 additive로 검토.
4. **CKV 문서 정정 요청(파일 소유=CKV)**: §3의 "coding-agent가 CKV 15도구를 본다"는 전제를
   "coding-agent는 CKS 13도구만 소비, CKV flow/invariant는 CKS 표면 노출 후 도달(parity 갭)"로 정정.
