...
coding-agent's `.coding-agent/tickets/{JIRA-ID}_{YYYYMMDD_HHMMSS}/` convention (state.json, plan.md, design-v{N}.md, test-report.md, logs/) is well-designed. **Keep it.** It is the trace sink for observability (§8).
...
- **Binary deterministic self-tests (stay in each repo):** recall@k, MRR, P/R/F1, citation accuracy, hallucination byte-check — pure computation against fixture ground truth. Run in CI. (ckv `eval` minus judge; ckg `internal/eval/retrieval`; cks `cks-eval`.)
...
|---|---|---|
| **Stage 1 — Jira ticket TEMPLATE format** | **was unowned → ADD** | Author `coding-agent/contract/jira-ticket.template.md` defining required fields per ticket type (feature/bugfix/code_review/release); `template-parse` validates against it and reports `missing_fields`. Parsing was kept; the *template definition* was missing. |
...
| **Stage 8 — PROTECTED folder** | **convention owned; protection unowned** | `.coding-agent/tickets/{id}_{ts}/` naming+logs kept (C5). **ADD** protection semantics: write-once perms / no-overwrite guard on the per-ticket dir so artifacts/logs aren't clobbered. Owner: coding-agent (state-machine skill). |
