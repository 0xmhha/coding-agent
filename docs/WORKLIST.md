...
| P1 도메인팩 계약 | `domain-pack.json`+`project_id`, `stablenet-*` 정적 호명 → 활성 팩 해석, chainbench→`mcp:` 스테이지 일반화 | ① 코어 grep-clean(도메인 용어 0) ② go-stablenet 무회귀(fcore-baseline) ③ Phase1 라이브 신뢰성 | ◐ **설계 ACCEPTED (Phase 0, 06-22)** — `docs/domain-pack-contract-adr-2026-06-22.md`. 결정 4건 확정: §3 로더+런타임 Read 메커니즘 승인(능력 확실/신뢰성=Phase1), 추출범위=invariants+context만, 위치=`plugin/domains/`, Project B 안 만듦→**Project A 무회귀+grep-clean 검증**. **구현(Phase 1~3, 교란·대형)은 별도 세션/승인 게이트 유지.** 구현 🔴 미착수 |
