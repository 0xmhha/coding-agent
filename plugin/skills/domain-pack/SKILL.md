---
name: domain-pack
description: "프로젝트-불문 도메인팩 로더. state.json의 project_id로 활성 프로젝트의 domain-pack(plugin/domains/{project_id}/)을 런타임에 해석해, 경로→모듈 분류·복잡도 추정·항상-켜진 invariants backstop을 제공한다. 도메인 *지식*은 그 프로젝트의 cks 인덱스가 권위. (ADR docs/domain-pack-contract-adr-2026-06-22.md)"
type: skill
---
...
pack       = Read(plugin/domains/{project_id}/domain-pack.json)  # 매니페스트
```
...
> 무관하다. 기존 패턴(state-machine의 state.json Read, 구 stablenet-context의 cks 라이브 호출)과
> 동일하다. (ADR §3.1)

## 2. 제공 절차

### 2.1 classify_domain(file_paths, symbols)
`Read(plugin/domains/{project_id}/{pack.context_classifier})` 의 경로→모듈 규칙(§"경로 기반 모듈
...
`Read(plugin/domains/{project_id}/{pack.invariants})` 의 불변식 목록을 **검색 결과와 무관하게**
