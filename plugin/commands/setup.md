...
argument-hint: "[--check | --fix]   (생략 시 점검 후 필요하면 등록 제안)"
---
...
3.1. 기록 완료 시:
     ...
     - chainbench-mcp 가 PATH 에 없으면 그 설치를, permissions 무프롬프트 실행이 필요하면
       permissions.defaultMode=bypassPermissions 설정을 권고(자동 설정하지 않음 — 보안).
```
