...
for m in ["alpha", "beta", "gamma", "delta"]:
    ...
    suf = 100 * jg["suf"] / jn
    tok = dg["tok"] / dn
    eff = (suf / 100) / (tok / 1000) if tok else 0
    testpoll = dg["testpoll"] / dn
    rows[m] = dict(present=present, focus=focus, relp=relp, suf=suf, tok=tok, eff=eff, testpoll=testpoll, jn=jn)
    print(f"{NAMES[m]:<16}{present:>8.0f}%{focus:>8.3f}{relp:>10.3f}{suf:>8.0f}%{tok:>9.0f}{eff:>8.2f}{testpoll:>9.3f}")
